...
' DATE  : 8/24/2012
'
' COMMENT: Finds all local user accounts, generates random passwords 
...
'Sets the password filename to the computer name for easy tracking purposes
filename = strComputer & "-PWLIST.txt"

'Goes through each user name found on the machine
For Each objUser In colAccounts
	'Saves the user name to an arry used in the password list file
	...
	Set objUser = GetObject("WinNT://" & strComputer & "/" & uName(i))
	objUser.SetPassword newpass
	objUser.SetInfo

	'Creates the information used for the password list file
...
'Prints the password file to the default printer
PrintFile(filename)
